...
export function selectStrategyActionPlan(configuration, payload, runtime) {
    ...
    const primary = selected
        ...
        .find((block) => block.action !== "variable" && runtime.isTrainableBlock(block) && runtime.actionExecutableNow(block, state)) ?? null;
    const plan = { primary };
    for (const { block: selectedBlock } of selected) {
        for (const block of runtime.normalizedBlockActions(selectedBlock).map((entry) => ({ ...selectedBlock, ...entry }))) {
            ...
            const executionHead = action.head === "movement" ? "movement" : action.head === "rotation" ? "rotation" : "ability";
