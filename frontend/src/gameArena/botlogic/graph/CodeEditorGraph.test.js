...
test("editor graph starts empty instead of spawning nodes from the payload", () => {
    const source = configuration({ type: "expression", left: "my.hp", comparator: "lt", right: { type: "variable", value: "opponent.hp" } });
...
test("compact editor chips preserve bounded user-facing names without changing semantics", () => {
    const graph = sanitizeCodeEditorGraph({
        ...createCodeEditorGraph(),
        variables: [{ id: "variable", kind: "variable", name: "My health", variableId: "my.hp" }],
    ...
    assert.equal(graph.variables[0].variableId, "my.hp");
...
test("a literal remains inline while a connected variable replaces only operand two", () => {
    const source = configuration({ type: "expression", left: "my.hp", comparator: "lt", right: { type: "number", value: 30 } });
    const graph = editorGraphForConfiguration(source);
    const variable = { id: "variable-opponent", kind: "variable", variableId: "opponent.hp" };
    ...
    assert.deepEqual(compiled.roots[0].branches[0].conditions[0].right, { type: "variable", value: "opponent.hp" });
});

test("boolean variables stay on the boolean True/False comparison and cannot feed numeric operand two", () => {
    const source = configuration({ type: "expression", left: "target.alive", comparator: "eq", right: { type: "boolean", value: true } }, { action: "lock_on", actionTarget: "opponent" });
    const booleanNode = { id: "boolean-node", kind: "variable", variableId: "target.alive" };
...
test("connections use named ports and reject ambiguous duplicates", () => {
    const source = configuration({ type: "expression", left: "my.hp", comparator: "lt", right: { type: "number", value: 30 } });
    const graph = editorGraphForConfiguration(source);
    const first = { id: "variable-first", kind: "variable", variableId: "my.hp" };
    const second = { id: "variable-second", kind: "variable", variableId: "opponent.hp" };
