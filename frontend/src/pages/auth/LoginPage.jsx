...
function UsernameSetupModal({ username, setUsername, error, isSubmitting, onSubmit, onClose }) {
    ...
    useDialogFocus(dialogRef, { initialFocusRef: usernameRef, onClose, lockScroll: true });
    return (
        <div ref={dialogRef} className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/75 px-5 py-6" role="dialog" aria-modal="true" aria-labelledby="username-setup-title" tabIndex={-1}>
                ...
                <form onSubmit={onSubmit} className="mt-5 space-y-4">
                    <div className="block text-left">
                        <input
                            ...
                            aria-describedby={error ? "google-username-error" : "google-username-help"}
                            placeholder="Username"
                            aria-label="Username"
                            className="w-full rounded border border-border-lo bg-zinc-950 px-3 py-3 text-sm text-ink-white outline-none placeholder:text-slate-400 focus:border-cyan-500"
                        />
                    </div>
                    <p id="google-username-help" className="text-xs text-slate-500">3–20 characters: letters, numbers, underscores, and hyphens only.</p>
                    {error && <p id="google-username-error" className="form-error text-sm text-red-300" role="alert">{error}</p>}
